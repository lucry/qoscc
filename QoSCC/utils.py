...
class TimeSequence:
    def __init__(self, hw, pw):
        self.buffer = []
        self.latest_hw_seq = []
        self.increment = []
        self.history = []
        self.his_ptr = 0
        self.hw = hw  # history window size
        self.pw = pw  # predict window size

    def store(self, data):
        self.increment.append(data)
        self.history.append(data)
        self.latest_hw_seq.append(data)
        hlen = np.shape(self.latest_hw_seq)[0]
        if (hlen>self.hw):
            self.latest_hw_seq = self.latest_hw_seq[(hlen-self.hw):]
    ...
    def get_dataset(self, sequence):
        X = []
        Y = []
        len_in = len(sequence)
        for i in range(len_in -self.hw-self.pw):
            X.append(sequence[i: i + self.hw])
            Y.append(sequence[i + self.hw: i + self.hw + self.pw])
